...
function convertChatBlocks(content: string): string {
  ...
  return content.replace(
    ...
    (
      ...
      return `<div class="chat ${user} ${emotion} ${pos}"><img src="${icon}" alt="${user} ${emotion}" class="chat-icon w-[120px] h-[120px] rounded-full" width="120" height="120"/><div class="chat-bubble">${body}</div></div>`;
    },
  );
}
