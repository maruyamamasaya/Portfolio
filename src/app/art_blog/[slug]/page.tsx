...
import { getPost, getSortedPosts, getBacklinks } from '@/lib/artPosts';
  ...
  try {
    ...
    return (
      <div className="blog-container win7-container">
        <BlogNavButtons />
        <article className="prose win7-main">
            {post.image && (
          ...
          {backlinks.length > 0 && (
            <div className="mt-8">
              ...
              <ul className="list-disc pl-5">
                {backlinks.map(link => (
                  <li key={link.slug}>
                    <Link href={`/art_blog/${link.slug}`}>{link.title}</Link>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <section className="space-y-4 win7-sidebar">
          <LeftSidebar />
