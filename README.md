...
Digi Goose は、作品（Works）と記事（Journal）を軸にしたポートフォリオサイトです。将来の CMS 再構成を見据え、記事と制作実績を共通運用で投稿・更新できる構成へ移行する方針です。
...
- ✏️ Markdown 記事の投稿・編集（内製 CMS）
- 🧭 作品（Works）との統合運用を前提にした構成
- 🗂️ カテゴリ・タグによる記事管理（再設計中）
- 📱 レスポンシブ対応
- 🔍 作品・記事の発見導線を改善する探索フロー
...
将来は `works/` を追加し、記事と同じ編集体験で制作実績を管理する構成を想定しています。
...
今回の作業進行は [`PORTFOLIO_CMS_ROADMAP.md`](PORTFOLIO_CMS_ROADMAP.md) で管理し、デザイン統一と CMS 基盤の土台化を順番に進めます。
