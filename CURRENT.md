...
公開サイト、記事探索、問い合わせ、ローカル記事編集は実装済み。2026-09-09 にトップページ、ヘッダー、フッターを画像欠落に依存しないポートフォリオ構成へ刷新し、サイト情報を `src/config/site.ts` に集約した。トップは、自己紹介、制作領域、仕事の進め方、発信、問い合わせの順で商談時に説明できる構成である。欠落画像91件は `data/image-assets.json` のマッピングにより既存SVGへ一時転送され、後日 `replacementUrl` だけで差し替えられる。`blog/` には `.gitkeep` しかなく、トップと記事ページは明示的な準備中表示になる。
...
4. `IMAGE_ASSETS.md` を見ながら本画像URLを設定し、About、Services、Works、Pricing、Contact の内容を順次棚卸しする。
