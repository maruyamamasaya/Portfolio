...
- 直近は「認可境界の本線化（記事/Works の API + revalidate）」を完了し、次は運用手順への落とし込みを進める。
...
1. 変更 API と revalidate の認可テスト（401/200）を追加し、失敗時のエラーメッセージを検証する。
2. 問い合わせレート制限と監査ログ要件（本番運用向け）を決める。
