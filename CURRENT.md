...
公開サイト、記事探索、問い合わせ、ローカル記事編集は実装済み。2026-09-09 にトップページ、ヘッダー、フッターを画像欠落に依存しない構成へ刷新し、サイト情報と主要画像参照を `src/config/site.ts` に集約した。`blog/` には `.gitkeep` しかなく、トップと記事ページは明示的な準備中表示になる。
...
- トップページ、ヘッダー、フッターは実在する SVG とテキスト主体で構成し、共通ナビゲーション、連絡先、SNS、トップ用画像を設定ファイルから参照する。
