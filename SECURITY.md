...
- `middleware.ts` と `src/middleware.ts` は `/developer_edit` とその配下だけを HTTP Basic 認証する。
- `/api/revalidate` は query parameter を `REVALIDATE_SECRET` と比較する。
- `/api/contact` は入力形式・長さ・honeypot と、送信元 IP ごとのプロセス内 rate limit を検査してから AWS SES を呼ぶ。
- `/api/posts` と `/api/posts/[filename]` は filename を検証するが、認証・認可は実装していない。
- Markdown の表示は生成 HTML を `dangerouslySetInnerHTML` へ渡す。記事入力元の信頼条件を変更する場合は sanitization を再評価する。

## 既知のリスク

1. 編集ページの Basic 認証を通らなくても、記事変更 API を直接呼び出せる。
2. Basic 認証は TLS 自体を提供しない。本番 transport の構成はリポジトリ外で判断不能。
3. 問い合わせの rate limit は再起動で消え、複数 instance で共有されない。
4. Browser editor と再検証 secret の契約が一致していない。secret を client bundle へ移して解決してはならない。
5. 重複 middleware のどちらが実環境で有効か、検証記録がない。
